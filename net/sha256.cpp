...
constexpr std::uint32_t rotate_right(std::uint32_t value, std::uint32_t shift) {
...
public:
    void update(const std::uint8_t* input, std::size_t length) {
        for (std::size_t index = 0; index < length; ++index) {
            block_[block_length_++] = input[index];
    ...
    std::string finish() {
        const std::uint64_t total_bits = bit_length_ + block_length_ * 8;
        ...
        for (int shift = 56; shift >= 0; shift -= 8) block_[block_length_++] = total_bits >> shift;
        transform();

        std::ostringstream digest;
        for (const auto value : state_) digest << std::hex << std::setfill('0') << std::setw(8) << value;
        return digest.str();
    }
...
private:
    void transform() {
        ...
        for (std::size_t index = 16; index < words.size(); ++index) {
            const auto sigma0 = rotate_right(words[index - 15], 7) ^ rotate_right(words[index - 15], 18) ^ (words[index - 15] >> 3);
            const auto sigma1 = rotate_right(words[index - 2], 17) ^ rotate_right(words[index - 2], 19) ^ (words[index - 2] >> 10);
        ...
        for (std::size_t index = 0; index < words.size(); ++index) {
            ...
            const auto first = h + sum1 + choice + kConstants[index] + words[index];
...
std::optional<std::string> sha256_file(const std::filesystem::path& path) {
    ...
    return file.bad() ? std::nullopt : std::optional<std::string>(hash.finish());
}
